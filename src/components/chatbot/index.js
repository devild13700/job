...
function Chatbot() {
  ...
  const chat = async () => {
    try {
      ...
      const callback = 'workwise AI offline! please try after some time.'
      setMessages((prevMessages) => [...prevMessages, { callback, type: 'ai' }])
    }
  }
      ...
      >
        ...
        >
          {messages.map((message, index) => (
            <div
              key={index}
              className={`flex gap-3 my-4 text-gray-600 text-sm flex-1 ${message.type === 'ai' ? 'items-start' : 'items-end justify-end'}`}
            >
              ...
              <p className='leading-relaxed'>
                <span className='block font-bold text-gray-700'>
                  {message.type === 'ai' ? 'Workwise AI' : 'You'}
                </span>{' '}
                {message.text}
              </p>
            </div>
